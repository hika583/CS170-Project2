...
// Evaluate a feature set using LOOCV + nearest neighbor
static double evaluateFeatureSet(const vector<Instance>& data,
                                 const vector<int>& features,
                                 Validator& v)
{
    // verbose = false here
    return v.leaveOneOutValidation(data, features, false);
}
...
void forwardSelection(const vector<Instance>& data,
                      int totalFeatures,
                      Validator& v)
{
    ...
    // initial message with empty set baseline (optional)
    double baseAcc = evaluateFeatureSet(data, currentSetOfFeatures, v);
    ...
    for (int level = 1; level <= totalFeatures; ++level) {
        ...
        for (int i = 0; i < (int)remainingFeatures.size(); ++i) {
            ...
            double accuracy = evaluateFeatureSet(data, candidateSet, v);
...
void backwardElimination(const vector<Instance>& data,
                         int totalFeatures,
                         Validator& v)
{
    ...
    double currentAccuracy     = evaluateFeatureSet(data, currentSetOfFeatures, v);
